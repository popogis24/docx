...
mypath = fr'C:\Users\Ander\OneDrive\Documentos\JORGE_DOCUMENTOS\LAGINHA\PDF_Desatualizados'
...
for pdf in range(len(list_pdf)):
    ...
    VALOR_TERRANUA_fl = float(AREA_TOTAL) * 9704
    VALOR_TERRANUA = locale.format_string("%.2f", VALOR_TERRANUA_fl, grouping=True)

    VALOR_IMOVEL_fl = float(VALOR_TERRANUA_fl) + float(VALOR_BENFEITORIA_fl)
    ...
    doc = docx.Document(fr'C:\Users\Ander\OneDrive\Documentos\JORGE_DOCUMENTOS\Modelo_Laginha.docx')
    #salva esse arquivo com o nome do pdf
    doc.save(fr'C:\Users\Ander\OneDrive\Documentos\JORGE_DOCUMENTOS\LAGINHA\DOC_Atualizados\{nome_pdf}.docx')
    #abre esse arquivo usando o 
    doc = DocxTemplate(fr'C:\Users\Ander\OneDrive\Documentos\JORGE_DOCUMENTOS\LAGINHA\DOC_Atualizados\{nome_pdf}.docx')
